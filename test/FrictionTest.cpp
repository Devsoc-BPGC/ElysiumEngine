...
int main() {
  ...
  bodyB.position =
      Vec3(1.9f, 0.0f, 0.0f); // Slight overlap to trigger collision
  ...
  // With friction, it should be less than 5.0f because it's opposing the
  // tangent motion.
  assert(bodyA.linearVelocity.y < 5.0f);

  // Check if it actually reduced
  if (bodyA.linearVelocity.y < 5.0f) {
    std::cout << "Friction test passed! Tangent velocity reduced from 5.0 to "
              << bodyA.linearVelocity.y << std::endl;
  } else {
    std::cout << "Friction test failed! Tangent velocity did not reduce."
              << std::endl;
    return 1;
  }
