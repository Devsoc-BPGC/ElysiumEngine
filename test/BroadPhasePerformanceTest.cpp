...
BruteForceGeneratePairs(const std::vector<RigidBody *> &bodies) {
...
void RunBenchmark(int numBodies) {
  std::cout << "\n--- Benchmarking BroadPhase with " << numBodies
            << " bodies ---" << std::endl;
  ...
  std::cout << "Brute Force: " << durationBrute << " us (" << pairsBrute.size()
            << " pairs found)" << std::endl;
  std::cout << "Spatial Hashing: " << durationOptimized << " us ("
            << pairsOptimized.size() << " pairs found)" << std::endl;

  if (durationOptimized < durationBrute) {
    float speedup = (float)durationBrute / durationOptimized;
    std::cout << "Speedup: " << speedup << "x" << std::endl;
  } else {
    std::cout << "Optimization is SLOWER by "
              << (float)durationOptimized / durationBrute << "x" << std::endl;
  }
  ...
  std::sort(pairsOptimized.begin(), pairsOptimized.end(),
            [](const CollisionPair &a, const CollisionPair &b) {
              if (a.A != b.A)
                return a.A < b.A;
              return a.B < b.B;
  ...
  if (pairsBrute.size() != pairsOptimized.size()) {
    std::cout << "ERROR: Pair count mismatch! Brute: " << pairsBrute.size()
              << ", Optimized: " << pairsOptimized.size() << std::endl;
  }
