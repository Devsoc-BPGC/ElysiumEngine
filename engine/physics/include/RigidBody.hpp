/**
 * @file RigidBody.hpp
 * @brief Defines the RigidBody structure and associated Axis-Aligned Bounding
 * Box (AABB) for physics simulation.
 */
...
/**
 * @struct RigidBody
 * @brief Represents a physical object with mass, velocity, and rotational
 * properties.
 */
struct RigidBody {
  ...
  Mat3 inverseInertiaTensorWorld; /**< Inverse inertia tensor transformed into
                                     world space. */

  Vec3 globalCentroid; /**< The center of mass in world coordinates. */
  Vec3 localCentroid;  /**< The center of mass relative to the body's local
                          origin. */
  ...
  Vec3 forceAccumulator; /**< Sum of all linear forces to be applied in the next
                            step. */
  Vec3 torqueAccumulator; /**< Sum of all torques to be applied in the next
                             step. */

  ColliderList
      colliders; /**< List of collision geometries attached to this body. */
  ...
  RigidBody()
      : mass(0.0f), inverseMass(0.0f), friction(0.3f),
        ...
        angularVelocity(0, 0, 0), forceAccumulator(0, 0, 0),
        torqueAccumulator(0, 0, 0) {}

  /** @name Transformation Synchronizers */
  ///@{
  /** @brief Updates the global centroid based on current position and
   * orientation. */
  ...
  /** @brief Updates the rotation matrices (orthonormalization and inverse
   * calculation). */
  ...
  /** @brief Transforms a direction vector from local space to world space (no
   * translation). */
  const Vec3 LocalToGlobalVec(const Vec3 &v) const;
  /** @brief Transforms a direction vector from world space to local space (no
   * translation). */
  ...
  AABB GetAABB() const {
    ...
    for (size_t i = 1; i < colliders.size(); ++i) {
      ...
      if (worldPos.x - r < box.min.x)
        box.min.x = worldPos.x - r;
      if (worldPos.y - r < box.min.y)
        box.min.y = worldPos.y - r;
      if (worldPos.z - r < box.min.z)
        box.min.z = worldPos.z - r;
      if (worldPos.x + r > box.max.x)
        box.max.x = worldPos.x + r;
      if (worldPos.y + r > box.max.y)
        box.max.y = worldPos.y + r;
      if (worldPos.z + r > box.max.z)
        box.max.z = worldPos.z + r;
    }
    return box;
  }
};
