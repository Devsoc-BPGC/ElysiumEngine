/**
 * @file CoreMath.hpp
 * @brief Essential linear algebra structures including vectors, matrices, and
 * quaternions.
 */
...
/** @brief Small value used for floating-point equality comparisons and
 * singularity checks. */
...
struct Mat3 {
   ...
   * @return The inverse matrix, or Identity if the matrix is singular (det
   * approx 0).
   */
  Mat3 Inverted() const {
  ...
  static Mat3 OuterProduct(const Vec3 &u, const Vec3 &v) {
    return Mat3(u.x * v.x, u.x * v.y, u.x * v.z, u.y * v.x, u.y * v.y,
...
/**
 * @brief Detailed implementation for converting a rotation matrix to a
 * quaternion.
