 ...
 * @brief Container class for managing rigid bodies, global forces, and
...
/**
 * @class PhysicsWorld
 * @brief Manages the collection of rigid bodies and orchestrates the simulation
 * steps.
 */
class PhysicsWorld {
public:
  ...
  /** @brief Global acceleration vector (e.g., Earth gravity at -9.81 on the
   * Y-axis). */
  Vec3 gravity;
   ...
   * @param rotation The rotation of the boundary box in radians (default is
   * 0.0f).
   */
  void SetBoundaries(const Vec3 &center, const Vec3 &halfExtents,
                     float rotation = 0.0f) {
   ...
   * @param gravityVector The initial global gravity force. Defaults to
   * Earth-like gravity.
   */
   ...
   * @note The world does not take ownership of the memory; the caller must
   * manage deletion.
   * @param body Pointer to the RigidBody instance to simulate.
   */
  void AddBody(RigidBody *body) { rigidBodies.push_back(body); }
  ...
  void RemoveBody(RigidBody *body) {
    rigidBodies.erase(std::remove(rigidBodies.begin(), rigidBodies.end(), body),
                      rigidBodies.end());
  }
  ...
  /**
   * @brief Resets the force and torque accumulators for all bodies in the
   * world.
   */
  void ClearForces();
    ...
    for (auto *body : rigidBodies) {
      ...
      for (auto &col : body->colliders) {
        ...
        if (localPos.y + col.radius > boundaryHalfExtents.y) { // Ceiling hit
          ...
          if (localVel.y > 0)
            localVel.y *= -restitution;
          subCollided = true;
        } else if (localPos.y - col.radius <
                   -boundaryHalfExtents.y) { // Floor hit
        ...
        if (localPos.x + col.radius > boundaryHalfExtents.x) { // Right Wall hit
          ...
          if (localVel.x > 0)
            localVel.x *= -restitution;
          subCollided = true;
        } else if (localPos.x - col.radius <
                   -boundaryHalfExtents.x) { // Left Wall hit
        ...
        if (subCollided) {
          ...
          body->position =
              boundaryCenter + (rot * localPos) -
              body->orientation.ToQuat().RotateVector(col.localCentroid);
