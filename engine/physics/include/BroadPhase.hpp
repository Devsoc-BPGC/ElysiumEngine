...
/**
 * @class BroadPhase
 * @brief A static utility class used to detect potential overlap between rigid
 * bodies using AABBs.
 * * The Broad Phase is the first stage of the collision pipeline. It uses
 * simple Axis-Aligned Bounding Boxes (AABBs) to perform fast intersection
 * tests, avoiding expensive narrow-phase geometric checks for objects that are
 * clearly separated.
 */
class BroadPhase {
public:
   ...
   * @return A vector of CollisionPair objects containing pointers to bodies
   * that might be colliding.
   */
  static std::vector<CollisionPair>
  GeneratePairs(const std::vector<RigidBody *> &bodies) {
    if (bodies.size() < 2)
      return {};
