 ...
 * 2. **Integration**: Updates velocities and positions based on accumulated
 * forces.
 * 3. **Broad Phase**: Prunes objects that are too far apart to collide,
 * returning potential pairs.
 * 4. **Narrow Phase**: Performs detailed collision checks and resolves impulses
 * between bodies.
 * 5. **Constraint Resolution**: Keeps bodies within the world boundaries (box
 * constraints).
...
void PhysicsWorld::Step(float dt) {
  ...
  std::vector<CollisionPair> candidates =
      BroadPhase::GeneratePairs(rigidBodies);
  ...
  // Final check to ensure balls stay within the simulation box after
  // collisions.
  ResolveBoundaries();
  ...
  if (!pairs.empty()) {
    // std::cout << "Broad Phase found " << pairs.size() << " potential
    // collisions!" << std::endl;
  }
}
