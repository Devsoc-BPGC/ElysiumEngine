...
class Input {
public:
  enum class ButtonState { None, Pressed, Held, Released };
  ...
  static void HandleEvent(const sf::Event &event) {
    if (const auto *keyPressed = event.getIf<sf::Event::KeyPressed>()) {
      ...
      m_mousePosition = {(float)mouseMoved->position.x,
                         (float)mouseMoved->position.y};
    } else if (const auto *mouseWheel =
  ...
  static bool IsMouseButtonDown(sf::Mouse::Button button) {
    ...
    return it != m_mouseButtonStates.end() &&
  ...
  static bool IsMouseButtonReleased(sf::Mouse::Button button) {
    auto it = m_mouseButtonStates.find(button);
    return it != m_mouseButtonStates.end() &&
           it->second == ButtonState::Released;
  }
...
private:
  ...
  static inline std::unordered_map<sf::Mouse::Button, ButtonState>
      m_mouseButtonStates;
